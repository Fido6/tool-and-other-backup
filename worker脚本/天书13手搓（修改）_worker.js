...
天书13手搓节点使用说明如下：
  ...
  二、v2rayN客户端的单节点路径设置（只是本单节点生效）
    1、/s5all=xxx（socks5代理所有网站）;
    2、/socks5=xxx（socks5代理cf网站，非cf网站走直连）;
    3、/pyip=xxx（pyip代理cf网站，非cf网站走直连）;
    三种任选其一，如果不设置留空，cf网站无法访问;
  注意：workers部署，最好绑定自定义域名，手搓443系6个端口节点;
*/
...
async function startTransferPipeline(ws, url) {
  ...
  try {
    ws.addEventListener('message', async event => {
      if (!firstPacket) {
        firstPacket = true;
        firstPacketReady = await parseFirstPacket(event.data);
    ...
    async function parseFirstPacket(firstData) {
      const bin = new Uint8Array(firstData);
      const formatVLKey = (a, i = 0) => [...a.slice(i, i + 16)].map(b => b.toString(16).padStart(2, '0')).join('').replace(/(.{8})(.{4})(.{4})(.{4})(.{12})/, '$1-$2-$3-$4-$5');
      if (formatVLKey(bin.slice(1, 17)) !== USER_KEY) throw new Error('UUID verification failed');
      const portIndex = 18 + bin[17] + 1;
      const destPort = new DataView(bin.buffer, portIndex, 2).getUint16(0);
      if (destPort === 53) { // handle DNS-over-TCP tunneled queries
        const dnsQuery = bin.slice(portIndex + 9);
      ...
      addrType = bin[addrIndex];
      let addrInfoIndex = addrIndex + 1;
      switch (addrType) {
        case 1:
          addrLen = 4;
          destHost = bin.slice(addrInfoIndex, addrInfoIndex + addrLen).join('.');
          break;
        case 2:
          addrLen = bin[addrInfoIndex];
          addrInfoIndex += 1;
          const domain = new TextDecoder().decode(bin.slice(addrInfoIndex, addrInfoIndex + addrLen));
          destHost = domain;
          break;
        case 3:
          addrLen = 16;
          const ipv6 = [];
          const readIPv6 = new DataView(bin.buffer, addrInfoIndex, 16);
      ...
      if (socksALLMatchSpec) {
        ...
        try {
          ...
          const pyipMatch = tempPath.match(/pyip\s*=\s*([^&]+(?::\d+)?)/i);
      ...
      const initialPayload = bin.slice(addrInfoIndex + addrLen);
      ...
      while (true) {
        ...
        if (returnData && returnData.length > 0) {
          if (ENABLE_FLOW_CONTROL) {
            let offset = 0;
            let chunkSize = FLOW_CHUNK_SIZE;
            while (offset < returnData.length) {
              const chunk = returnData.slice(offset, offset + chunkSize);
